# Academic Portfolio | Seneca College
 
Welcome to my academic portfolio for CEP146!
 
## About Me
- Name: Jisun Park
- Program: Computer Programming and Analysis (CPA)
- Focus: C Programming, Linux/Unix Systems, Version Control
- GitHub: [@jisunpark-ca](https://github.com/jisunpark-ca)

## Tech Stack
- Languages: C
- Tools: Git, GitHub, Visual Studio, VS Code
- OS: Windows, Linux

### Semester 1 (Fall 2025)
* **IPC144:** C Programming
    * *Focus:* Structured Programming, Arrays, Pointers, Memory Management
* **OPS102:** Operating Systems for Programmers (Unix & Windows)
    * *Focus:* Command Line Interface (CLI), Shell Scripting, File Systems
* **MTH140:** Mathematics for Programmers
    * *Focus:* Functions, Permutations, Combinations, Probability
* **APS145:** Applied Problem Solving
    * *Focus:* Computational Thinking, Flowcharts, Pseudo Code
* **CEP146:** Tools & AI
    * *Focus:* Git & GitHub, Markdown, AI Fundamentals

## Goals
- [ ] Master C programming fundamentals (Pointers, Memory Management).
- [ ] Proficiently use Linux CLI and write Shell Scripts.
- [ ] Apply mathematical concepts to programming logic.
- [ ] Understand AI ethics and utilize AI tools effectively.
- [x] Establish a professional Git & GitHub workflow.
 
## Projects
*This section will be updated as I complete assignments*
